...
import it.cnr.istc.iloc.types.propositionalstate.PropositionalStateType;
 ...
 */
class PropositionalStateTimelineVisualizer implements ITimelineVisualizer {

    @Override
    public Class<? extends IType> getTimelineType() {
        return PropositionalStateType.class;
    }
    ...
    public XYPlot getPlot(IModel model, IObject object) {
        ...
        ValueXYIntervalSeries positive_states = new ValueXYIntervalSeries("Positives");
        ValueXYIntervalSeries negative_states = new ValueXYIntervalSeries("Negatives");

        ArrayList<Double> ends = new ArrayList<>(10);
        ends.add(0.0);

        object.getType().getPredicates().values().stream().flatMap(predicate -> predicate.getInstances().stream().map(instance -> (IFormula) instance)).filter(formula -> formula.getFormulaState() == FormulaState.Active && formula.get(model, Constants.SCOPE) == object).sorted((IFormula f0, IFormula f1) -> Double.compare(model.evaluate((INumber) f0.get(Constants.START)).doubleValue(), model.evaluate((INumber) f1.get(Constants.START)).doubleValue())).forEach(formula -> {
            ...
            boolean positive = formula.getType().getName().endsWith("True");
            double y = getYValue(start, end, ends);
            if (positive) {
                positive_states.add(start, start, end, y, y - 1, y, model, formula);
            } else {
                negative_states.add(start, start, end, y, y - 1, y, model, formula);
            }
        });

        collection.addSeries(positive_states);
        collection.addSeries(negative_states);
