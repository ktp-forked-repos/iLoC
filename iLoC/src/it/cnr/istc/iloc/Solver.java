...
public class Solver implements ISolver {
    ...
    private int bound;
    private final Deque<INode> breached = new LinkedList<>();
    ...
    public boolean solve() {
        this.bound = predicates.size() + staticCausalGraph.getTypes().stream().mapToInt(type -> type.getPredicates().size()).sum();
    ...
    public void solveFlaw(IFlaw flaw) {
        ...
        resolvers.stream().forEach(resolver -> {
            INode child = new Node((Node) currentNode);
            n_nodes++;
    ...
    public String toString() {
        ...
        predicates.values().stream().flatMap(type -> type.getInstances().stream()).map(instance -> (IFormula) instance).collect(Collectors.toList()).forEach(formula -> {
        ...
        types.values().stream().flatMap(type -> type.getPredicates().values().stream()).flatMap(type -> type.getInstances().stream()).map(instance -> (IFormula) instance).collect(Collectors.toList()).forEach(formula -> {
            switch (formula.getFormulaState()) {
                case Inactive:
                    solver.append("I ");
                    break;
